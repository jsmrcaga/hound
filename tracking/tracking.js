...
tracking.track = function(campaign, req, usr=null){
	let user = req.cookies['hound-user'] || usr || utils.genUUID();
	campaign.track(req.query.hound_ref, req.query.hound_source, req.query.hound_medium, req.query.hound_extra || null, user);
	return {
...
tracking.pixel = function(pixel, req){
	...
	pixel.track(user);
	return tracking.track(campaign, req, user);
};
